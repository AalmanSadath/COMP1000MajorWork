...
public void setup() {
    ...
    for(int i=0;i<=5;i++){
        pedArr[i][0]=PApplet.parseInt(random(width/5+25,(width*4/5)-25)); //x coordinate of ped
        pedArr[i][1]=-20; //y coordinate of ped starts offscreen to provide smoothness
        pedArr[i][2]=PApplet.parseInt(random(2,5)); //speed of ped
        pedArr[i][3]=PApplet.parseInt(random(0,256));
        pedArr[i][4]=PApplet.parseInt(random(0,256));
        pedArr[i][5]=PApplet.parseInt(random(0,256));
    }
    for(int i=0;i<=2;i++){
...
public void drawTree(int x, int y, float scale, int bulbs, float radius){
    for(int i=1;i<=bulbs;i++){
        ...
        float yCordBranch=radius*sin(2*n*3.14f/bulbs)+y; //gets y ^^
        strokeWeight(5);
        stroke(79, 57, 9);
...
public void treeTransfer(){ //when 4th tile moves off screen, all characteristics of trees are moved one index above in array
    for(int i=3;i>0;i--){
